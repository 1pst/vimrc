...
function! s:Vbookmark_IsHas()
	if '' == findfile(g:vbookmark_bookmarkSaveFile)
		return 1;
	endif
	let l:str = readfile(g:vbookmark_bookmarkSaveFile, '', 1)
	let l:local = stridx(l:str[0],"/")
	if l:local == -1
		return 0
	else
		return 1
...
function! s:Vbookmark_generateSignId()
	if !exists('s:vbookmark_signSeed')
		let s:vbookmark_signSeed = 201306
	endif
...
""""echo "s:vbookmark_curGroupIndex" s:vbookmark_curGroupIndex
endfunction
...
""modified by fred 
	let l:idx = strridx(g:vbookmark_bookmarkSaveFile,"/")
	"let l:str =finddir(strpart(g:vbookmark_bookmarkSaveFile,0,l:idx))
	"disable recursive
	let l:str =finddir(strpart(g:vbookmark_bookmarkSaveFile,0,l:idx),strpart(g:vbookmark_bookmarkSaveFile,0,l:idx),0)
	if (strlen(l:str) > 0)
		call writefile([outputGroups, outputCurGroupIndex], g:vbookmark_bookmarkSaveFile)
		if s:Vbookmark_IsHas() == 0 
			call delete(g:vbookmark_bookmarkSaveFile)
		endif
	endif
endfunction
...
if !exists(':VbookmarkTest')
	command -nargs=0 VbookmarkTest :call s:Vbookmark_IsHas()
endif
...
if !exists('g:vbookmark_disableMapping')
	...
	"nnoremap <silent> md :VbookmarkTest<CR>
